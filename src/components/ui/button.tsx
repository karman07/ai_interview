...
export default function Button({ children, onClick, variant = "primary", className = "" }: ButtonProps) {
  const base = "px-6 py-3 rounded-2xl font-semibold shadow-lg transition-all duration-300 inline-flex items-center justify-center relative overflow-hidden group transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-opacity-20";
  ...
  const variants: Record<string, string> = {
    ...
    secondary: "bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 shadow-amber-500/25 hover:shadow-amber-500/40 focus:ring-amber-500 before:absolute before:inset-0 before:bg-white before:opacity-0 before:transition-opacity before:duration-300 hover:before:opacity-10",
    ghost: "bg-transparent text-indigo-600 border-0 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 hover:text-indigo-700 shadow-none hover:shadow-lg focus:ring-indigo-500",
