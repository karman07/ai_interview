...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800/60 px-4 h-16 flex justify-between items-center">
      ...
      <div
        className={cn(
          "lg:hidden fixed top-0 left-0 h-full w-80 max-w-[85vw] z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto bg-white dark:bg-[#0B1120]",
      ...
      >
        <div className="p-6 pt-20">
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800/60">
            <button
              onClick={toggleTheme}
              className="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:border-slate-200 dark:hover:border-slate-700 border border-transparent transition-all duration-200"
            >
              <div className="flex-shrink-0 p-1.5 rounded-md text-slate-600 dark:text-slate-400">
          ...
          {/* Logout Button */}
          <div className="mt-2 border-t border-slate-200 dark:border-slate-800/60">
            <button
            ...
            >
              <div className="flex-shrink-0 p-1.5 rounded-md text-slate-600 dark:text-slate-400 group-hover:text-red-600 dark:group-hover:text-red-400">
                <LogOut size={18} />
              </div>
              <div className="flex-1 min-w-0 text-left">
                <span className="text-sm font-medium text-slate-900 dark:text-slate-100 group-hover:text-red-600 dark:group-hover:text-red-400">
                  Logout
                </span>
                <p className="text-xs text-slate-500 dark:text-slate-400">
      ...
      <div className="hidden lg:block w-64 flex-shrink-0">
        <aside
          className="fixed top-0 left-0 w-64 h-screen border-r border-slate-100 dark:border-slate-800/40 bg-white dark:bg-[#0B1120] overflow-y-auto no-scrollbar shadow-sm"
        >
          <div className="p-6">
            ...
            {/* Theme Toggle */}
            <div className="mt-8 pt-6 border-t border-slate-100/10 dark:border-slate-800/50">
              <button
                onClick={toggleTheme}
            ...
            {/* Logout Button */}
            <div className="mt-2 border-t border-slate-100/10 dark:border-slate-800/50">
              <button
                onClick={handleLogout}
