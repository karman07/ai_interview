...
export class InterviewService {
  ...
  async getHistory(userId: string): Promise<InterviewDocument[]> {
    return this.interviewModel.find({ userId }).sort({ createdAt: 1 }).exec();
  }
  ...
  // 👉 Fixed version: fully resets round
  async resetRound(userId: string, round: string): Promise<DeleteResult> {
    return this.interviewModel.deleteMany({ userId, round }).exec();
  }
  ...
  async getHistoryForRound(
    ...
    return this.interviewModel.find({ userId, round }).sort({ createdAt: 1 }).exec();
  }

  async getResultsForRound(userId: string, round: string) {
    const interviews = await this.interviewModel.find({ userId, round }).exec();
    return this.buildResults(interviews);
  }
